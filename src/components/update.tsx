...
interface IProps {
  ...
  setNhanvien: (value: Nhanvien | null) => void;
}
function UpdateModel(props: IProps) {
  const { showModelUpdate, setShowModelUpdate, nhanvien, setNhanvien } = props;
  ...
  const handleSubmit = () => {
    ...
    fetch(`http://127.0.0.1:8000/api/nhanvien/${id}`, {
      headers: {
    ...
    .then(res => {
      ...
      if (res.headers.get("content-length") === "0" || !res.headers.get("content-type")?.includes("application/json")) {
        throw new Error("Phản hồi từ server không chứa nội dung JSON hợp lệ");
      }
      return res.json();
    })
    .then(data => {
  ...
  return (
    <>
      <Modal show={showModelUpdate} onHide={() => handleCloseModel()} size="lg">
